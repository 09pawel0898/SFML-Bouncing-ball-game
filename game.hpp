...
class Game
{
	...
	public:
		...
		void getMenuStateEvents(void);

		//Game State
		void renderGameState(void);
		void updateGameState(void);
		void getGameStateEvents(void);

		//Lost Game State
		void renderLostState(void);
		void updateLostState(void);
		void getLostStateEvents(void);

		//return true if cursor is covering the button, false otherwise
		bool buttonCover(sf::Sprite button, int radius, sf::Vector2i mouseposition, std::string buttontype);
};
