...
void Game::initSprites(void)
{
    ...
    {
        this->ballicon[i].setTexture(this->ballicontexture); this->ballicon[i].setOrigin(42, 42); this->ballicon[i].setScale(0.5f, 0.5f);
    }
    for (unsigned int i = 0; i < 6; i++)
...
void Game::getMenuStateEvents(void)
{
...
void Game::getGameStateEvents(void)
{
	while (this->window->pollEvent(this->event))
	{
        /* window closed */
...
void Game::getLostStateEvents(void)
{
	while (this->window->pollEvent(this->event))
	{
        /* window closed */
...
bool Game::buttonCover(sf::Sprite button, int radius, sf::Vector2i mouseposition, std::string buttontype)
{
    double mx, my, bx, by;
    mx = mouseposition.x;
    my = mouseposition.y;
